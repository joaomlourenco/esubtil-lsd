...
public class LoadDataWorker implements Runnable
{
	...
	for (int d_id = 1; d_id <= 10; d_id++)
	{
		...
		else
		{
		    ...
		    stmtCustomer.setTimestamp(13, new Timestamp(System.currentTimeMillis()));
		...
		else
		{
		    ...
		    stmtOrder.setTimestamp(5, new Timestamp(System.currentTimeMillis()));
		    if (o_id < 2101)
		...
		for (int ol_number = 1; ol_number <= o_ol_cnt; ol_number++)
		{
			...
			if (o_id < 2101)
			    stmtOrderLine.setTimestamp(7, new Timestamp(now));
			else
			    stmtOrderLine.setNull(7, Types.TIMESTAMP);
